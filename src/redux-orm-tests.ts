...
(() => {
     ...
     */
    const authorModel = Person.create({
        id: "A1",
        firstName: "A1",
        lastName: "A1",
    });
...
(() => {
     ...
     * In case of MutableQuerySets/many-to-many relationships, an array of union of above-mentioned types is accepted
     */
    const authorModel = Person.upsert({
        id: "A1",
        firstName: "A1",
        lastName: "A1",
    });
...
(() => {
    ...
    const ormSelector = createOrmSelector(
        orm,
        (session) => session.Book.all().toRefArray()[0]
    );
...
(() => {
    ...
    const selector4 = createOrmSelector(
        ...
        (s) => s.foo,
        (s) => s.bar,
        (session, id, title, id2, title2) =>
            session.Book.get({ id, title, id2, title2 })!.ref
    ) as TestSelector;
